...
app.config['WTF_CSRF_ENABLED'] = False  # Disabled for development; enable in production
app.config['WTF_CSRF_TIME_LIMIT'] = None  # No time limit for CSRF tokens
# Disable redirect pages - use HTTP 302 redirects directly
app.config['SEND_FILE_MAX_AGE_DEFAULT'] = 0

# Configure secure session cookies
# Note: SECURE=False in development (HTTP), True in production (HTTPS)
app.config['SESSION_COOKIE_SECURE'] = False  # Allow HTTP in development
app.config['SESSION_COOKIE_HTTPONLY'] = True  # Prevent JavaScript access
app.config['SESSION_COOKIE_SAMESITE'] = 'Strict'  # CSRF protection - Strict required for CSRF token validation
...
def create_2fa_verification(user_id, email, user_name=None, role='admin'):
    ...
    try:
        ...
        db.rollback()
        print(f'❌ Failed to create 2FA verification: {exc}')
        import traceback
        traceback.print_exc()
        return None, None
    finally:
...
def archived_applicants():
    """HR view: list archived applicants (applications with status 'archived' or archived_at set)."""
    db = get_db()
    if not db:
        flash('Database connection error.', 'error')
    ...
    cursor = db.cursor(dictionary=True)
    try:
        # Determine archiving strategy: prefer status='archived' or archived_at column
        archived_clause = "(a.status = 'archived')"
        try:
            cursor.execute('SHOW COLUMNS FROM applications')
            cols = {row.get('Field') for row in cursor.fetchall() or []}
            if 'archived_at' in cols:
                archived_clause = "(a.status = 'archived' OR a.archived_at IS NOT NULL)"
        except Exception:
            pass

        query = f"""
            SELECT a.application_id, a.applicant_id, a.resume_id, a.status, a.applied_at AS submitted_at,
                   COALESCE(a.archived_at, a.applied_at) AS archived_at,
            ...
            ORDER BY COALESCE(a.archived_at, a.applied_at) DESC
        """
        ...
        print(f'❌ Archived applicants error: {exc}')
        import traceback
        traceback.print_exc()
...
def delete_applicant_permanent():
    ...
    cursor = db.cursor()
    try:
        # Delete related interviews and notifications first
        try:
            cursor.execute('DELETE FROM interviews WHERE application_id = %s', (application_id,))
        except Exception:
            pass
...
def archive_applicant(application_id):
    ...
    try:
        ...
        cursor.execute(
            "UPDATE applications SET status = 'archived', archived_at = NOW() WHERE application_id = %s",
            (application_id,)
        )
        if cursor.rowcount > 0:
            db.commit()
...
def delete_applicant_now(application_id):
    ...
    cursor = db.cursor()
    try:
        # Delete related interviews and notifications first (best-effort)
        try:
            cursor.execute('DELETE FROM interviews WHERE application_id = %s', (application_id,))
        except Exception:
            pass
...
def admin_download_resume(resume_id):
    ...
    try:
        ...
        if not (absolute_path == upload_base_real or absolute_path.startswith(upload_base_real + os.sep)):
...
def admin_view_resume(resume_id):
    ...
    try:
        ...
        if user.get('role') == 'hr':
            ...
            if branch_id:
                cursor.execute(
                    ...
                    JOIN jobs j ON a.job_id = j.job_id
                    JOIN applicants ap ON a.applicant_id = ap.applicant_id
                    WHERE a.applicant_id = %s AND ap.applicant_id = %s AND j.branch_id = %s
                    LIMIT 1
                    ''',
                    (record['applicant_id'], record['applicant_id'], branch_id),
                )
                if not cursor.fetchone():
                    flash('You can only access resumes from your branch.', 'error')
                    return redirect(url_for('applicants'))
        ...
        # Always send inline for viewing (PDFs) and force download for others
        download_name = record.get('file_name') or os.path.basename(absolute_path)
        file_ext = os.path.splitext(download_name.lower())[1]
        if file_ext == '.pdf':
            mimetype_value = 'application/pdf'
        ...
        # Read file safely (small files only) and return Response with security headers
        from flask import Response
...
def applicants():
    ...
    try:
        ...
        # EXCLUDE archived applicants from stats
    ...
    try:
        ...
        if applications:
            for application in applications:
                try:
                    interview_count = application.get('interview_count', 0) or 0
                    status_value = (application.get('status') or 'pending').strip().lower()
        ...
        # Remove duplicates by resume_id
        seen_ids = set()
        unique_resumes = []
        ...
        view_template = 'hr/view_applicant_fixed.html' if user.get('role') == 'hr' else 'admin/view_applicant.html'
